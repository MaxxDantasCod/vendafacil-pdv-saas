<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Editar Loja</h2>
                <p class="text-sm text-gray-600">Altere dados e planos da loja.</p>
            </div>
            <div>
                    ...
                    <form method="POST" action="{{ route('admin.tenants.update', $tenant) }}">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <div>
                                ...
                                <input id="name" name="name" value="{{ old('name', $tenant->name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                            </div>

                            <div>
                                <label for="email" class="block text-sm font-medium text-gray-700">Email</label>
                                <input id="email" name="email" type="email" value="{{ old('email', $tenant->email) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                            </div>

                            <div>
                                <label for="dolibarr_url" class="block text-sm font-medium text-gray-700">URL do Dolibarr</label>
                                <input id="dolibarr_url" name="dolibarr_url" type="url" value="{{ old('dolibarr_url', $tenant->dolibarr_url) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                            </div>

                            <div>
                                <label for="dolibarr_db" class="block text-sm font-medium text-gray-700">Base de dados Dolibarr</label>
                                <input id="dolibarr_db" name="dolibarr_db" value="{{ old('dolibarr_db', $tenant->dolibarr_db) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select id="plan_status" name="plan_status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                    <option value="active" {{ old('plan_status', $tenant->plan_status) === 'active' ? 'selected' : '' }}>Ativo</option>
                                    <option value="pending" {{ old('plan_status', $tenant->plan_status) === 'pending' ? 'selected' : '' }}>Aguardando pagamento</option>
                                    <option value="overdue" {{ old('plan_status', $tenant->plan_status) === 'overdue' ? 'selected' : '' }}>Atrasado</option>
                                    <option value="cancelled" {{ old('plan_status', $tenant->plan_status) === 'cancelled' ? 'selected' : '' }}>Cancelado</option>
                                </select>
                            </div>

                            <div>
                                <label for="api_key" class="block text-sm font-medium text-gray-700">API Key Dolibarr</label>
                                <input id="api_key" name="api_key" value="{{ old('api_key', $tenant->api_key) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                            </div>
                        </div>
